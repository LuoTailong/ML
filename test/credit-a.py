...
# 1 导入数据
sale1Data = pd.read_csv("data/a-可选消费品（多指标）-训练.csv.csv", sep=",", error_bad_lines=False)
...
# y = sale1Data["17"]
x = sale1Data.drop(columns=["ck", "a", "b"], axis=1)
y = sale1Data["ck"]
...
export_graphviz(dtc, out_file="creditLevel.dot", feature_names=x.columns,
                class_names=["C", "CC", "BB", "A-", "A", "A+", "AA-", "AA", "AA+", "AAA"], filled=True)
